...
import useRequest from '../useRequest/useBaseRequest';
...
export default function useSearchPeople() {
  ...
  var _useCloudJwt = useCloudJwt(jwtToken, useJwtToken),
      _useCloudJwt2 = _slicedToArray(_useCloudJwt, 1),
      token = _useCloudJwt2[0];

  var fetchOptions = useCallback(_async(function (_ref) {
  ...
  var _useRequest = useRequest(function (v) {
  ...
  var search = useDebounceFn(_async(function (v) {
    if (v === '') return;
    getData(v);
    return _await();
  }), wait || 500).run;

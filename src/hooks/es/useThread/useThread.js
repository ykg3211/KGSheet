...
var useThread = function useThread(fn) {
  ...
  var _useState = useState(THREAD_TASK_STATUS.PENDING),
  ...
  var setWorkerStatus = useCallback(function (status) {
    isRunning.current = status === THREAD_TASK_STATUS.RUNNING;

    _setWorkerStatus(status);
  }, []);
  var killWorker = useCallback(function () {
    var _worker$current;
  ...
  var onWorkerEnd = useCallback(function (status) {
    killWorker();
    setWorkerStatus(status);
  }, [killWorker, setWorkerStatus]);
  var generateWorker = useCallback(function () {
    var remoteDependencies = options.remoteDependencies,
        timeout = options.timeout;
    var blobUrl = createWorkerBlobUrl(fn, remoteDependencies);

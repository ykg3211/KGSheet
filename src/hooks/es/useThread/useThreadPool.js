...
var useThreadPool = function useThreadPool(fn, options) {
  ...
  var workerUrlRef = useRef('');
  var workerPoolRef = useRef(new Set());
  var freeWorkersRef = useRef([]);
  var allTasksRef = useRef(new Map());
  var pendingQueueRef = useRef([]);
  var poolSize = options.size || navigator.hardwareConcurrency / 2; // 设置worker任务状态

  var setTaskStatus = useCallback(function (id, status, result) {
  ...
  var onTaskFinish = useCallback(function (worker) {
  ...
  var startWorker = useCallback(function (resolve, reject, fnArgs, id) {
    var task = {
  ...
  var initWorkerPool = useCallback(function () {
    if (poolSize < 1) {
  ...
  var runFn = useCallback(function () {
  ...
  var killTask = useCallback(function (id, status) {
    var task = allTasksRef.current.get(id);
  ...
  var getTaskStatus = useCallback(function (id) {
    var task = allTasksRef.current.get(id);
  ...
  var getFreeCount = useCallback(function () {
    return freeWorkersRef.current.length;
  }, []); // 获取等待任务数

  var getPendingCount = useCallback(function () {
    return pendingQueueRef.current.length;
  }, []); // 获取执行任务数

  var getRuningCount = useCallback(function () {

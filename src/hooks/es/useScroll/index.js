...
import { INITIAL_DATA, DEFAULT_TIMEOUT, getDirectionX, getDirectionY, getTotalDistanceX, getTotalDistanceY, getRelativeDistanceX, getRelativeDistanceY, getSpeedX, getSpeedY } from './utils';

var useScroll = function useScroll(el) {
  var options = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
  var deps = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : [];

  var _useState = useState(INITIAL_DATA),
  ...
  var raf = useRef(null);
  var timer = useRef();
  var beginData = useRef(INITIAL_DATA);
  var currentData = useRef(INITIAL_DATA);
  var beginTimestamp = useRef();
  var currentTimestamp = useRef();
  ...
  var frameHandler = function frameHandler(timestamp) {
    ...
    var direction = {
      x: getDirectionX(offset.x, currentData.current),
      y: getDirectionY(offset.y, currentData.current)
    };
    var totalDistance = {
      x: getTotalDistanceX(offset.x, currentData.current),
      y: getTotalDistanceY(offset.y, currentData.current)
    };
    var relativeDistance = {
      x: getRelativeDistanceX(offset.x, beginData.current),
      y: getRelativeDistanceY(offset.y, beginData.current)
    };
    var timestampDiff = timestamp - (currentTimestamp.current || 0);
    var speed = {
      x: getSpeedX(offset.x, currentData.current, timestampDiff),
      y: getSpeedY(offset.y, currentData.current, timestampDiff)
    };
  ...
  var onScrollEnd = function onScrollEnd() {
    currentData.current = _objectSpread(_objectSpread(_objectSpread({}, currentData.current), INITIAL_DATA), {}, {
  ...
  var clearAndSetTimer = function clearAndSetTimer() {
    ...
    timer.current = window.setTimeout(onScrollEnd, DEFAULT_TIMEOUT);
  };

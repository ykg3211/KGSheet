...
export default class EditCellPlugin {
	...
	private initPlugin() {
		...
		if (this._this[PluginTypeEnum.ExcelBaseFunction]) {
			...
			console.error(
				'CommonInputPlugin 依赖于 ExcelBaseFunction, 请正确注册插件!',
			);
		}
	}
			...
			(ctx) => {
				...
				if (this.startRegularCell) {
					...
					const columnGap = Math.max(
						Math.abs(currentCell.column - centerTag.column) -
							Math.abs(startCell.column + endCell.column) / 2,
						0,
					);
					const rowGap = Math.max(
						Math.abs(currentCell.row - centerTag.row) -
							Math.abs(startCell.row - endCell.row) / 2,
						0,
					);

					if (columnGap >= rowGap) {
						this.regularArrow =
							currentCell.column > centerTag.column
								? regularArrowEnum.LEFT2RIGHT
								: regularArrowEnum.RIGHT2LEFT;
						currentCell.row = endCell.row;
					} else {
						this.regularArrow =
							currentCell.row > centerTag.row
								? regularArrowEnum.TOP2BOTTOM
								: regularArrowEnum.BOTTOM2TOP;
						currentCell.column = endCell.column;
					}

					const { leftTopCell, rightBottomCell } = combineCell([
						startCell,
						endCell,
						currentCell,
					]);
					...
					this.drawDashBorder(
						ctx,
						this._this.calBorder(leftTopCell, rightBottomCell),
					);
				...
				if (this.startCopyCell && this.pointDownCell) {
					const { leftTopCell, rightBottomCell } =
						this.getCurrentScopeInCopy() || {};
	...
	private initEvent() {
		...
		this._this.setEvent(EventConstant.MOUSE_DOWN, {
			...
			judgeFunc: (e) => {
				if (
					this.editDomInstance &&
					!(e as any).path.includes(this.editDomInstance)
				) {
					this.removeDom();
				}
				return false;
		...
		this._this.setEvent(EventConstant.DB_CLICK, {
			...
			judgeFunc: (e) => {
				...
				if (
					...
					this.SelectPlugin._startCell.column !==
						this.SelectPlugin._endCell.column
				) {
					return false;
				}
	...
	private getCurrentScopeInCopy() {
		...
		const { startCell: leftTopCell, endCell: rightBottomCell } = deepClone(
			this.startCopyCell,
		);
		if (this.currentCell && this.pointDownCell) {
			...
			columnGap = Math.min(
				columnGap,
				this._this._data.w.length - rightBottomCell.column,
			);
			rowGap = Math.min(
				rowGap,
				this._this._data.h.length - 1 - rightBottomCell.row,
			);
	...
	private handleCopyCB() {
		...
		for (
			let row = sourceCells.startCell.row;
			row <= sourceCells.endCell.row;
			row++
		) {
			const temp: cell[] = [];
			for (
				let column = sourceCells.startCell.column;
				column <= sourceCells.endCell.column;
				column++
			) {
		...
		Object.keys(SourceData.data.spanCells).forEach((key) => {
			...
			newKey[0] +=
				targetCells.leftTopCell.row - SourceData.scope.leftTopCell.row;
			newKey[1] +=
				targetCells.leftTopCell.column - SourceData.scope.leftTopCell.column;

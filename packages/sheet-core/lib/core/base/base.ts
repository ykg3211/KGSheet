...
class Base extends Render {
	...
	public calBorder(startCell: selectedCellType, endCell: selectedCellType) {
		...
		const cellPosition: rectType = [
			_data.w.slice(0, startCell.column).reduce((a, b) => a + b, 0) +
				paddingLeft -
				scrollLeft,
			_data.h.slice(0, startCell.row).reduce((a, b) => a + b, 0) +
				paddingTop -
				scrollTop,
			_data.w
	 ...
	 */
	public transformXYInContainer(
		e: MouseEvent,
		alwayInner = false /** 如果点在外面则平移到视图内部 */,
	) {
		if (!this.canvasDom) {
			return false;
		}
		const result = [
			e.pageX - this.canvasDom.offsetLeft,
			e.pageY - this.canvasDom.offsetTop,
		];
		if (
			result[0] > 0 &&
			result[1] > 0 &&
			result[0] < this._width &&
			result[1] < this._height
		) {
		...
		source.forEach((c) => {
			if (
				judgeCross(
					...
					[
						(cell as selectedCellType).column,
						(cell as selectedCellType).row,
						w,
						h,
					],
				)
	...
	public getCellByPoint(point: [number, number]) {
		...
		if (point[0] <= this.paddingLeft && point[1] <= this.paddingTop) {
			...
			renderCellsArr.some((row) => {
				if (row.length > 0) {
					if (
						point[1] >= row[0].point[1] &&
						point[1] < row[0].point[1] + row[0].h
					) {
						row.some((cell) => {
							if (
								judgeOver(point, [cell.point[0], cell.point[1], cell.w, cell.h])
							) {
								selectedCell = {
									row: cell.location.row,
									column: cell.location.column,
	...
	public getDataByScope({
		...
		const cells = this._data.cells
			.slice(leftTopCell.row, rightBottomCell.row + 1)
			.map((row) => {
		...
		for (let row = leftTopCell.row; row < rightBottomCell.row + 1; row++) {
			for (
				let column = leftTopCell.column;
				column < rightBottomCell.column + 1;
				column++
			) {
				if (this._data.spanCells[row + '_' + column]) {
					spanCells[row + '_' + column] =
						this._data.spanCells[row + '_' + column];
				}
			}
		}
	...
	public setDataByScope(SourceData: BaseDataType) {
		...
		for (let row = leftTopCell.row; row <= rightBottomCell.row; row++) {
			for (
				let column = leftTopCell.column;
				column <= rightBottomCell.column;
				column++
			) {
				this._data.cells[row][column] =
					data.cells[row - leftTopCell.row][column - leftTopCell.column];
				this._data.cells[row][column] =
					data.cells[row - leftTopCell.row][column - leftTopCell.column];
				// 用来撑大单元格的
	...
	public getSpanCell(cell: selectedCellType) {
		return this._data.spanCells[cell.row + '_' + cell.column] as
			| spanCell
			| undefined;
	}
}
